import { Paper, TableContainer, Table, TableHead, TableRow, TableCell, TableBody, Container, Button, Box, Typography, TextField, SelectChangeEvent, FormControl, Select, InputLabel, MenuItem, Slider } from "@mui/material";
...
function AllCompanyCoupons() {
  ...
  const handleCategoryChange = (event: SelectChangeEvent) => {
    ...
    const ShowDeleteButton = () => (<>
        <Container>
          <Box>
            <Typography>Please Insert ID Of Coupon To Delete</Typography><br/>
            <TextField required className="inputRounded" type="number" id="id" label="id" value={couponId}
              onChange={(e) => { setId(Number(e.target.value));   } } />
            <br/><br/>
            <Button variant="contained" value="Search" onClick={handleDeleteCoupon}> Submit </Button>
          </Box>
          </Container>
        </>)
...
const handleDeleteCoupon = () => {
    axios.delete<void>(globals.urls.deleteCoupon+`${couponId}`, {headers: {
    ...
    const handleAllCoupons = () => { 
        axios.get<CouponModel[]>(globals.urls.allCompanyCoupons, {headers: {
          authorization: `${localStorage.getItem('token')}`
        }})
        .then(res => {
            setCoupons(res.data);
